...
class Processor
{
    ...
    protected function processSave($object, $update = null, $commitWithin = null)
    {
        ...
        if ($classMetadata->callback) {
            if (!method_exists($object, $classMetadata->callback)) {
                throw new \InvalidArgumentException(sprintf('Method "%s" targeted by Callback does not exist', $classMetadata->callback));
            }

            $reflMethod = new \ReflectionMethod($object, $classMetadata->callback);
            $reflMethod->invoke($object, $document);
        }
